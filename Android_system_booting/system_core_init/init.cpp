...
int main(int argc, char** argv) {
    ...
    open_devnull_stdio();
	//初始化Kernel log,位于设备节点/dev/kmsg，可通过cat /dev/kmsg来获取内核log
	//
    klog_init();
	//设置输出的log级别
	//设置log的输出级别为KLOG_NOTICE_LEVEL(5)，当log级别小于5时则会输出到kernel log， 默认值为3.
    ...
    parser.ParseConfig("/init.rc");//解析init.rc文件,Zygote通过init.zygote64/32.rc启动
